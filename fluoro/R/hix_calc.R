...
hix.calc=function(corr.eem){
    ...
    ### WAY 2
    temp.exp=lapply(seq(from=300, to=max(as.numeric(rownames(corr.eem))), by=2),
                    function(u)
                        akima::aspline(x=as.numeric(colnames(corr.eem)),
                                       y = corr.eem[which(rownames(corr.eem)==u),],
    ...
    row.names(temp.exp.2)= seq(from=300, to=max(as.numeric(rownames(corr.eem))), by=2)

    rough.exp=lapply(seq(from=240, to=450),
           function(u)
               akima::aspline(x=as.numeric(rownames(temp.exp.2)),
                              y = temp.exp.2[,colnames(temp.exp.2)==u],
                              xout = seq(from=300, to=max(as.numeric(rownames(corr.eem))), by=1)
               )
    )
    ...
    colnames(expanded.eem)=seq(from=240, to=450, by=1)
    row.names(expanded.eem)= seq(from=300,  to=max(as.numeric(rownames(corr.eem))), by=1)
