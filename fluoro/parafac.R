...
#### R VERSION START
parafac=function(X,Fac,Options,const,OldLoad,FixMode,Weights){

    NumbIteraInitia=20 #                                                    Number of inital iterations

    if(missing(X)){stop("Input 'X' must be given!")}
    if(missing(Fac)){stop("Input 'Fac' must be given!")}


    DimX = dim(X) ###
    X = reshape(X,DimX(1),prod(DimX(2:end))) ##

    nonneg_obeyed = TRUE # used to check if noneg is ok

    if(nargin<3){
        #load noptiopf ####                                                 OTHER MLAB FUNCTION-NEED TO SOURCE
        OptionsDefault=Options
    }else{
        # Call the current Options OptionsHere and load default to use if some of the current settings should be default
        Options=Options #                                                   I believe this means load up the full options object,
    }
    if( I==length(Options)){
        #Options=zeros(8,1)
        Options=matrix(nrow = 8, ncol=1, data = 0) #                        We may be making an array of "false"s with the zero business
    }

    if (length(Options)<8){
        #Options=[Optionszeros(8-I,1)]                                          fix syntax
    ...
    if(exists(OldLoad)){
        OldLoad=0
    }else if(length(OldLoad)==0){                                           #possibly trying to remove old objects from env? if so, use rm()
        OldLoad=0
    }
    ########################################################################
    ...
    Plt=Options[3,1]
    if (Plt %in% c(0:3)){
        stop(' Options(3,1) - Plotting - not set correct must be 0,1,2 or 3')
    }
    ...
    if (showfit<(-1)|all.equal(round(showfit),showfit)){
        stop(' Options(5,1) - How often to show fit - not set correct must be positive integer or -1')
    }

    ### I GOT THIS FAR May 18 2018
...
# The i'th loading goes from lidx(i,1) to lidx(i,2)
                                                  ...
                                                  Factors{1}=AFactors{2}=BFactors{3}=C
                                                  ...
                                                  f=2*crit
                                                  it=0
                                                  connew=2conold=1 # for missing values
...
if ~any(FixMode)&~(any(const==2)|any(const==3))
  ...
  #   DIDNt WORK (TOOK AGES FOR 7WAY DATA)
                                                                        ...
                                                                        if isa(X,'dataset')# Then it's a SDO
                                                                                                             ...
                                                                                                             return(Factors,it,err,corcondia)
}
