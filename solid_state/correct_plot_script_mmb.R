...
for(g in 1:length(grps)){
    ...
    make.plot=function(info){
        ...
        eem.plot=eem.plot(corr.eem = p.eem.out, sample.name = info$name, save.dir = "../180516/corrected/")
        return(eem.plot)
    }

    plots=lapply(seq(nrow(info)), function(x) make.plot(info[x,]))
    gridded=gridExtra::grid.arrange(grobs=plots, ncol=2)
    ggsave(filename = paste0(sample,".png"), plot = gridded, device = "png", path = "../180516/corrected/", width = 8.5, units = "in", height = 11, dpi = 300)
}
